...
//        total_center_pane.setPadding(new Insets(30,50,30,50));
        total_center_pane.setBorder(new Border(new BorderStroke(Color.ROSYBROWN, BorderStrokeStyle.DASHED, null,new BorderWidths(3))));
        total_center_pane.setPrefSize(200, 1006);
        gailan_tab.setText("MAIN");
        func_tab.setText("DATALOAD");
        manage_data_tab.setText("CODE");
        personal_info_tab.setText("SETTINGS");
...
//        personal_info_tab.setGraphic(new ImageView(new Image(Main.class.getResourceAsStream("./resources/4.png"), TAB_SIZE, TAB_SIZE, false, true)));
        ...
        try {
            ...
            func_tab.setContent(data_load);
            manage_data_tab.setContent(edit_code);
        } catch (IOException e) {
            e.printStackTrace();
        }
...
//        gailan_tab.setContent(gailan_pane);
    ...
    private Pane loginAni() {
        ...
        rootContent.getTransforms().addAll(new Translate(Main.SCENE_WIDTH / 2, Main.SCENE_HEIGHT *2/5),new Rotate(225));
        JFXButton startBtn = new JFXButton("start");
        startBtn.setLayoutX(Main.SCENE_WIDTH *5/6 );
        startBtn.setLayoutY(Main.SCENE_HEIGHT *5/6);
        startBtn.setAlignment(Pos.BASELINE_LEFT);
        startBtn.setGraphicTextGap(16.0);

        Animator animator = new Animator(new TreeGenerator(treeContent, Main.NUMBER_OF_BRANCH_GENERATIONS), startBtn);
        animator.run();
